...
def get_unique_cart_key(call_id: Optional[str], agent_id: Optional[str], restaurant_id: Optional[int], session_id: Optional[str] = None) -> str:
    ...
    """
    # Real calls have unique call_ids - prefer over LLM-generated session_id
    if call_id and call_id not in ("playground", "test", "simulation"):
        return call_id

    # Session ID for test/playground calls - prefix with restaurant_id to prevent
    # cross-restaurant collisions (LLMs may generate the same 8-char ID)
    if session_id:
        prefix = f"r{restaurant_id}_" if restaurant_id else ""
        return f"session_{prefix}{session_id}"
