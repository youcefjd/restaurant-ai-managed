...
import { Phone, Save, AlertCircle, CheckCircle, Info, ExternalLink, Clock, Search, MapPin, Loader2, Trash2, Calendar } from 'lucide-react'
...
export default function Settings() {
  ...
  const [maxAdvanceOrderDays, setMaxAdvanceOrderDays] = useState(0)
...
// Update form fields when account data loads
  useEffect(() => {
    if (account) {
      ...
      if (account.max_advance_order_days !== undefined) {
        setMaxAdvanceOrderDays(account.max_advance_order_days)
      }
    }
  }, [account])
  ...
  // Mutation to update order settings
  const updateOrderSettingsMutation = useMutation({
    mutationFn: async (days: number) => {
      return restaurantAPI.updateOrderSettings(accountId!, days)
    },
    onSuccess: () => {
    ...
    onError: (err: any) => {
      setError(err.response?.data?.detail || 'Failed to update order settings')
      setSuccess('')
    },
  })
      ...
      <div className="bg-white rounded-2xl p-8 shadow-card border border-gray-100">
        <div className="flex items-start gap-4 mb-6">
          ...
          <div className="flex-1">
            ...
            <p className="text-gray-600">
              Configure how customers can place orders through the voice AI.
            </p>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div>
            <label htmlFor="advance-days" className="block text-sm font-semibold text-gray-700 mb-2">
            ...
            <div className="flex items-center gap-4">
              <select
                ...
                onChange={(e) => setMaxAdvanceOrderDays(Number(e.target.value))}
                className="px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all"
                disabled={updateOrderSettingsMutation.isPending}
              >
                <option value={0}>Same-day only</option>
                <option value={1}>Up to 1 day in advance</option>
                <option value={2}>Up to 2 days in advance</option>
                <option value={3}>Up to 3 days in advance</option>
                <option value={7}>Up to 1 week in advance</option>
              </select>
              <button
                type="button"
                onClick={() => updateOrderSettingsMutation.mutate(maxAdvanceOrderDays)}
                disabled={updateOrderSettingsMutation.isPending}
                className="px-6 py-3 bg-primary-600 text-white rounded-xl font-semibold hover:bg-primary-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center gap-2"
              >
            ...
            <p className="text-xs text-gray-500 mt-2">
              Controls how far in advance customers can schedule pickup orders. "Same-day only" means customers can only order for today.
            </p>
          </div>
          ...
          <div className="bg-gray-50 border border-gray-200 rounded-xl p-4">
            ...
            <p className="text-gray-900">
              {account?.max_advance_order_days === 0 || account?.max_advance_order_days === undefined
                ? 'Same-day orders only'
                : account.max_advance_order_days === 1
                ? 'Orders accepted up to 1 day in advance'
                : `Orders accepted up to ${account.max_advance_order_days} days in advance`}
            </p>
          </div>
        </div>
      </div>
